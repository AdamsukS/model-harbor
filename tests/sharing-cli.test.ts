...
test('sharing setup is repeatable, keeps secrets private, validates templates, and supports revocation', () => {
  ...
  try {
    ...
    config.models = { 'another-model': 131072, standard: 32768 };
    config.concurrency = 8;
    config.perKeyConcurrency = 8;
    writeFileSync(configPath, JSON.stringify(config));
    expect(cli('render').status).toBe(0);
    expect(cli('handoff').status).toBe(0);
    const profileGuide = readFileSync(guidePath, 'utf8');
    expect(profileGuide).toContain('`standard`（32768 tokens）');
    expect(profileGuide).toContain('最多 8 个在途请求');
    expect(profileGuide).not.toContain('{{');
    config.models.standard = 0;
    writeFileSync(configPath, JSON.stringify(config));
    expect(cli('render').status).toBe(1);
    config.models.standard = 32768;
